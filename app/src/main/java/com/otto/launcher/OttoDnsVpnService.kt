...
private object OttoWebsitePolicy {
    ...
    // Domain substrings that always trigger a block (catches any "*porn*" domain, etc.).
    private val blockedDomainSubstrings = setOf(
        "porn",
        "xxx",
        "hentai",
        "nsfw"
    ...
    fun isBlockedDomain(domain: String, context: Context): Boolean {
        ...
        if (blockedDomainSubstrings.any { normalizedDomain.contains(it) }) return true
