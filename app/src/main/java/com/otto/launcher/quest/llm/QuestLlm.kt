...
object QuestClassifier {
    suspend fun classify(raw: String): Result<ClassifiedQuest> = GroqClient.chatJson(
...
object QuestDrafter {
    suspend fun draft(summon: String, quests: List<QuestEntity>, defers: Map<QuestEntity, Int>): Result<QuestDraft> {
        ...
        return GroqClient.chatJson(rules, "Summon: $summon\nCandidates: $candidates").mapCatching(::parseDraft)
    }
...
object QuestStepPlanner {
    suspend fun plan(quest: QuestEntity, installedApps: Map<String, String>): Result<QuestPlan> {
        ...
        return GroqClient.chatJson("Create concrete ordered imperative steps. suggestedApps must contain only packages from the supplied installed list. Return {steps:[string],suggestedApps:[package]}.",
...
object QuestEndJudge {
    suspend fun judge(text: String): Result<QuestEndVerdict> = GroqClient.chatJson(
